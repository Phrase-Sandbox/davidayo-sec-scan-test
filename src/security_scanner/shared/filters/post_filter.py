...
module enforces the same rules **programmatically** as a backstop, in three
ordered passes:
...
3. **Low-confidence findings** — pre-gate they are dropped entirely. The
   skill (on-demand) report may surface them as informational; the gate does
   not.
...
from security_scanner.shared.models.enums import Confidence
